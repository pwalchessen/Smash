...
We explored the literature online, and discovered that SimHash was primarily designed for finding small changes on websites. Thus, the design principle of SimHash was to have a high resolution of changes when there are very few changes overall between two datasets. Some even suggested that SimHash cannot handle too big of changes or hamming distances, with say a hamming distance of over 20 causing SimHash to lose its resolution. Thus, we needed to find a way to make up for the loss of resolution.


\subsection{SMASH}
[1.5 pages] Thus came the development of original research based on what we learned and observed from MinHash and SimHash. Seeing how the existing SimHash algorithm was losing resolution when the Hamming distance was too great, we went back to the drawing board and examined each part of the SimHash algorithm closely. From the examination and with additional brainstorming, we came up with SMASH, which is designed to primarily address the shortcoming of resolution loss with SimHash.
